...
class HTMLDocumentBackend(DeclarativeDocumentBackend):
    def __init__(self, in_doc: "InputDocument", path_or_stream: Union[BytesIO, Path]):
        ...
        _log.debug("About to init HTML backend_temp...")
        self.soup = None
        # HTML file:
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not initialize HTML backend_temp for file with hash {self.document_hash}."
            ) from e
    ...
    def convert(self) -> DoclingDocument:
        ...
        if self.is_valid():
            ...
            raise RuntimeError(
                f"Cannot convert doc with {self.document_hash} because the backend_temp failed to init."
            )
        return doc
